<div>
    <flux:heading size="xl">Status Kawin</flux:heading>
    <flux:text class="mt-2">This Page Show List of status kawins</flux:text>

    <div class="relative overflow-x-auto bg-white rounded-lg shadow-md p-6 mb-4 mt-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex flex-col md:flex-row gap-4 flex-1">
                <div class="flex-1">
                    <input wire:model.live="search" type="text" placeholder="Search Status Kawin..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('tarif_pkp')">
                            <div class="flex items-center gap-2">
                                <span>Tarif PKP</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'tarif_pkp')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($statuskawins as $statuskawin)
                        <tr class="hover:bg-gray-50">
                            <td class="px-2 py-4 whitespace-nowrap text-center text-sm">
                                {{ $statuskawins->firstItem() + $loop->index }}.
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                                {{ $statuskawin->nama ?: '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $statuskawin->tarif_pkp ? 'Rp ' . number_format($statuskawin->tarif_pkp, 0, ',', '.') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $statuskawin->keterangan ?: '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                @if ($showDeleted)
                                    <div class="flex justify-center gap-2">
                                        <button wire:click="confirmRestore({{ $statuskawin->id }})"
                                        ...
                                        <button wire:click="confirmForceDelete({{ $statuskawin->id }})"
                                ...
                                @else
                                    <div class="flex justify-center gap-2">
                                        <button wire:click="edit({{ $statuskawin->id }})"
                                        ...
                                        <button wire:click="confirmDelete({{ $statuskawin->id }})"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">No Status Kawin found</p>
                                    <p class="text-sm">Get started by creating a new Status Kawin.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl font-semibold text-gray-900">
                                {{ $isEdit ? 'Edit Status Kawin' : 'Create Status Kawin' }}
                            </h3>
                    ...
                    <div class="px-6 py-6">
                        <form wire:submit.prevent="save" class="space-y-5">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    <label class="text-sm font-medium text-gray-700">
                                        Nama PKP <span class="text-red-500">*</span>
                                    </label>
                                    <input wire:model.live="nama" type="text" placeholder="Enter Nama/Kode PKP"
                                ...
                                <!-- Mata Pelajaran Code -->
                                <div class="space-y-2">
                                    <label class="text-sm font-medium text-gray-700">Tarif PKP <span class="text-red-500">*</span></label>
                                    <input type="text" wire:model="tarif_pkp"
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea wire:model="keterangan" rows="3" placeholder="sebutkan keterangan..."
