<div>
    <flux:heading size="xl">Tahun Ajaran</flux:heading>
    <flux:text class="mt-2">This Page Show List of Tahun Ajarans</flux:text>

    <div class="relative overflow-x-auto bg-white rounded-lg shadow-md p-6 mb-4 mt-4">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div class="flex flex-col md:flex-row gap-4 flex-1">
                <div class="flex-1">
                    <input wire:model.live="search" type="text" placeholder="Search Tahun Ajaran..."
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('awal_periode')">
                            <div class="flex items-center gap-2">
                                <span>Awal Periode</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'awal_periode')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('akhir_periode')">
                            <div class="flex items-center gap-2">
                                <span>Akhir Periode</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'akhir_periode')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @forelse($tahunAjarans as $ta)
                        <tr class="hover:bg-gray-50">
                            <td class="px-2 py-4 whitespace-nowrap text-center text-sm">
                                {{ $tahunAjarans->firstItem() + $loop->index }}.
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                                {{ $ta->periode ?: '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $ta->awal_periode ? \Carbon\Carbon::parse($ta->awal_periode)->format('d M Y') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $ta->akhir_periode ? \Carbon\Carbon::parse($ta->akhir_periode)->format('d M Y') : 'N/A' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button wire:click="toggleStatus({{ $ta->id }})"
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                {{ $ta->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $ta->is_active ? 'Aktif' : 'Nonaktif' }}
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                <div>{{ $ta->created_at->format('d M Y') }}</div>
                                <div class="text-xs text-gray-500">by {{ $ta->creator->name ?? 'System' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                @if ($showDeleted)
                                    <div class="flex justify-center gap-2">
                                        <button wire:click="confirmRestore({{ $ta->id }})"
                                        ...
                                        <button wire:click="confirmForceDelete({{ $ta->id }})"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">No Tahun Ajaran found</p>
                                    <p class="text-sm">Get started by creating a new Tahun Ajaran.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                    ...
                    <div class="px-6 py-5 border-b border-gray-100 bg-gray-50/50">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl font-semibold text-gray-900">
                                {{ $isEdit ? 'Edit Tahun Ajaran' : 'Create Tahun Ajaran' }}
                            </h3>
                    ...
                    <div class="px-6 py-6">
                        <form wire:submit.prevent="save" class="space-y-5">
                            ...
                            <div class="space-y-2">
                                ...
                                <input wire:model.live="periode" type="text" placeholder="Enter Periode name"
                                    class="w-full px-4 py-3 bg-white border rounded-lg text-sm focus:ring-2 transition-all @error('periode') border-red-500 focus:border-red-500 focus:ring-red-500/20 @else border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 @enderror">
                                @error('periode')
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="space-y-2">
                                    <label class="text-sm font-medium text-gray-700">
                                        Awal Periode <span class="text-red-500">*</span>
                                    </label>
                                    <input wire:model.live="awal_periode" type="date"
                                        class="w-full px-4 py-3 bg-white border rounded-lg text-sm focus:ring-2 transition-all @error('awal_periode') border-red-500 focus:border-red-500 focus:ring-red-500/20 @else border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 @enderror">
                                    @error('awal_periode')
                                        <p class="text-xs text-red-500 flex items-center gap-1">
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input wire:model.live="akhir_periode" type="date"
                                        class="w-full px-4 py-3 bg-white border rounded-lg text-sm focus:ring-2 transition-all @error('akhir_periode') border-red-500 focus:border-red-500 focus:ring-red-500/20 @else border-gray-200 focus:border-blue-500 focus:ring-blue-500/20 @enderror">
                                    @error('akhir_periode')
                                        <p class="text-xs text-red-500 flex items-center gap-1">
                                            <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd"
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea wire:model="keterangan" rows="3" placeholder="Keterangan Tahun Ajaran..."
                        ...
                        <div class="flex gap-3 justify-end">
                            ...
                            <button type="submit" wire:click="save"
                                ...
                                <span wire:loading.remove wire:target="save">
                                    {{ $isEdit ? 'Update Tahun Ajaran' : 'Create Tahun Ajaran' }}
                                </span>
