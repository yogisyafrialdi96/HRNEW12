<div>
    ...
    <div class="flex flex-column sm:flex-row flex-wrap sm:space-y-0 items-center justify-between">
        <div>
            ...
            <flux:text>This Page Show List of Bank Account</flux:text>
        </div>

        <div>
            @can('karyawan_bank.create')
            ...
            @else
                <div class="text-sm text-gray-500 italic">No permission to create Bank Account</div>
            @endcan
        </div>
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('nomor_rekening')">
                            <div class="flex items-center gap-2">
                                <span>Rekening</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'nomor_rekening')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @forelse($bankaccounts as $bank)
                        <tr class="hover:bg-gray-50">
                            <td class="px-2 py-4 whitespace-nowrap text-center text-sm">
                                {{ $bankaccounts->firstItem() + $loop->index }}.
                            </td>
                            <td class="px-6 py-4 w-72">
                                <div>
                                    <div class="text-sm font-medium text-gray-900 capitalize">
                                        {{ $bank->nama_bank  ?? '-'}}  ({{$bank->kode_bank ?? '-'}})</div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">
                                <button
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $bank->statusBadge['class'] }}">
                                    {{ $bank->statusBadge['text'] }}
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <button wire:click="toggleStatus({{ $bank->id }})"
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                                {{ $bank->is_primary ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $bank->is_primary ? 'Ya' : 'Tidak' }}
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if ($bank->document_path)
                                    <!-- Tombol untuk membuka modal preview -->
                                    <button
                                        wire:click="$dispatch('preview-file', { url: '{{ Storage::url($bank->document_path) }}' })"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-2">
                                    <!-- Detail Button -->
                                    <button wire:click="showDetail({{ $bank->id }})"
                        ...
                        <tr>
                            <td colspan="10" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">No bank found</p>
                                    <p class="text-sm">Get started by creating a new bank.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                        ...
                        <div class="px-6 py-6 max-h-[70vh] overflow-y-auto">
                            <form wire:submit.prevent="save" enctype="multipart/form-data" class="space-y-5">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <input wire:model.live="nama_bank" type="text"
                                        placeholder="e.g. Bank BRI, BCA, Mandiri"
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="kode_bank" type="number"
                                            placeholder="e.g. 002, 008, 014"
                                            class="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                                        @error('kode_bank')
                                            <p class="text-xs text-red-500 flex items-center gap-1">
                                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="nomor_rekening" type="number"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="nama_pemilik" type="text"
                                    ...
                                    <!--Jenis Rekening -->
                                    <div class="space-y-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Rekening
                                            <span class="text-red-500">*</span></label>
                                        <div class="relative">
                                            <select wire:model.live="jenis_rekening"
                                                ...
                                                <option value="" class="text-gray-400">Pilih Jenis Rekening
                                                </option>
                                                @foreach (\App\Models\Employee\KaryawanBankaccount::JENIS_REKENING as $key => $label)
                                    ...
                                    <!--Tujuan-->
                                    <div class="space-y-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Tujuan
                                            <span class="text-red-500">*</span></label>
                                        <div class="relative">
                                            <select wire:model.live="tujuan"
                                                class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all appearance-none">
                                                <option value="" class="text-gray-400">Pilih tujuan
                                                </option>
                                                @foreach (\App\Models\Employee\KaryawanBankaccount::TUJUAN_REKENING as $key => $label)
                                                    <option value="{{ $key }}">{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            <svg class="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="cabang" type="text"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="relative">
                                            <select wire:model.live="status"
                                                ...
                                                @foreach (\App\Models\Employee\KaryawanBankaccount::STATUS_REKENING as $key => $label)
                                                    <option value="{{ $key }}">{{ $label }}</option>
                                                @endforeach
                                            </select>
                                            <svg class="absolute right-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none"
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="tanggal_buka" type="date"
                                    ...
                                    <div class="space-y-2">
                                        <label class="text-sm font-medium text-gray-700">
                                            Akun Utama? <span class="text-red-500">*</span>
                                        </label>
                                        <div class="flex gap-4">
                                            <label class="flex items-center cursor-pointer">
                                                <input wire:model.live="is_primary" type="radio" value="1"
                                                    class="sr-only peer">
                                                <div
                                            ...
                                            <label class="flex items-center cursor-pointer">
                                                <input wire:model.live="is_primary" type="radio" value="0"
                                                    class="sr-only peer">
                                                <div
                            ...
                            <div class="flex flex-col-reverse sm:flex-row gap-3 justify-end">
                                ...
                                <button type="submit" wire:click="save"
                                    ...
                                    <span wire:loading.remove wire:target="save" class="flex items-center gap-2">
                                        ...
                                        {{ $isEdit ? 'Update Bank' : 'Simpan Bank' }}
                                    </span>
            ...
            <div x-transition:enter="transition ease-out duration-300 delay-100"
                ...
                <div
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="flex items-center space-x-2">
                            ...
                            <h2 class="text-xl font-bold text-gray-900 dark:text-white">Detail Akun Bank Karyawan</h2>
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        <button wire:click="closeModal"
                ...
                <div class="flex-1 overflow-y-auto p-4">
                    @if (!empty($selectedBank))
                        <!-- Bank Account Header -->
                        <div class="mb-6 p-4 bg-gradient-to-r from-sky-50 to-blue-50 dark:from-sky-900/20 dark:to-blue-900/20 rounded-lg border border-sky-200 dark:border-sky-800">
                            <div class="flex items-start justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center space-x-3 mb-2">
                                        <div class="flex-shrink-0 w-12 h-12 rounded-lg bg-white dark:bg-gray-700 flex items-center justify-center shadow-md">
                                        ...
                                        <div>
                                            <h3 class="text-xl font-bold text-gray-900 dark:text-white">
                                                {{ $selectedBank->nama_bank ?? 'Tidak Diketahui' }}
                                            </h3>
                                            <div class="flex items-center space-x-2 mt-1">
                                                <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full
                                                    @if($selectedBank->status === 'aktif') bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400
                                                    @elseif($selectedBank->status === 'nonaktif') bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400
                                                    @elseif($selectedBank->status === 'blocked') bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-400
                                                    @elseif($selectedBank->status === 'closed') bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400
                                                    @else bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400 @endif">
                                                    @if($selectedBank->status === 'aktif')
                                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6-2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                                        </svg>
                                                    @elseif($selectedBank->status === 'blocked')
                                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636m12.728 12.728L18.364 5.636M5.636 18.364l12.728-12.728" />
                                                        </svg>
                                                    @elseif($selectedBank->status === 'closed')
                                                        <svg class="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                                        </svg>
                                                    @endif
                                                    {{ ucfirst($selectedBank->status) }}
                                                </span>
                                                
                                                @if($selectedBank->is_primary)
                                    ...
                                    <div class="space-y-1 mt-3">
                                        <div class="flex items-center space-x-2">
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                                Kode: {{ $selectedBank->kode_bank ?? 'N/A' }}
                                            </span>
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded
                                                @if($selectedBank->jenis_rekening === 'tabungan') bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400
                                                @elseif($selectedBank->jenis_rekening === 'giro') bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400
                                                @elseif($selectedBank->jenis_rekening === 'deposito') bg-purple-100 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400
                                                @elseif($selectedBank->jenis_rekening === 'kredit') bg-orange-100 text-orange-700 dark:bg-orange-900/20 dark:text-orange-400
                                                @else bg-gray-100 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400 @endif">
                                                {{ ucfirst($selectedBank->jenis_rekening) }}
                                            </span>
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded
                                                @if($selectedBank->tujuan === 'gaji') bg-emerald-100 text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400
                                                @elseif($selectedBank->tujuan === 'bonus') bg-amber-100 text-amber-700 dark:bg-amber-900/20 dark:text-amber-400
                                                @elseif($selectedBank->tujuan === 'tunjangan') bg-cyan-100 text-cyan-700 dark:bg-cyan-900/20 dark:text-cyan-400
                                                @elseif($selectedBank->tujuan === 'reimburse') bg-indigo-100 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400
                                                @else bg-gray-100 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400 @endif">
                                                💰 {{ ucfirst($selectedBank->tujuan) }}
                                            </span>
                                        </div>
                                        @if($selectedBank->cabang)
                                            <p class="text-sm text-gray-600 dark:text-gray-400">
                                                🏦 Cabang: {{ $selectedBank->cabang }}
                                            </p>
                                        @endif
                                    </div>
                                </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                                <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                                    <h4 class="font-semibold text-gray-900 dark:text-white">Detail Rekening</h4>
                                </div>
                                <div class="p-4 space-y-3">
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Nomor Rekening</dt>
                                        <dd class="mt-1 text-base text-gray-900 dark:text-gray-100 font-mono font-bold bg-gray-50 dark:bg-gray-700 px-3 py-2 rounded">
                                            {{ $selectedBank->nomor_rekening ?? 'N/A' }}
                                        </dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Nama Pemilik</dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 font-medium">{{ $selectedBank->nama_pemilik ?? 'N/A' }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Nama Bank</dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $selectedBank->nama_bank ?? 'N/A' }}</dd>
                                    </div>
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Kode Bank</dt>
                                        <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100 font-mono">{{ $selectedBank->kode_bank ?? 'N/A' }}</dd>
                                    </div>
                                    @if($selectedBank->cabang)
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Cabang</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $selectedBank->cabang }}</dd>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                                <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                                    <h4 class="font-semibold text-gray-900 dark:text-white">Jenis & Status</h4>
                                </div>
                                <div class="p-4 space-y-3">
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Jenis Rekening</dt>
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full
                                                @if($selectedBank->jenis_rekening === 'tabungan') bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400
                                                @elseif($selectedBank->jenis_rekening === 'giro') bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400
                                                @elseif($selectedBank->jenis_rekening === 'deposito') bg-purple-100 text-purple-800 dark:bg-purple-900/20 dark:text-purple-400
                                                @elseif($selectedBank->jenis_rekening === 'kredit') bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400
                                    ...
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Tujuan Penggunaan</dt>
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full
                                                @if($selectedBank->tujuan === 'gaji') bg-emerald-100 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-400
                                                @elseif($selectedBank->tujuan === 'bonus') bg-amber-100 text-amber-800 dark:bg-amber-900/20 dark:text-amber-400
                                                @elseif($selectedBank->tujuan === 'tunjangan') bg-cyan-100 text-cyan-800 dark:bg-cyan-900/20 dark:text-cyan-400
                                                @elseif($selectedBank->tujuan === 'reimburse') bg-indigo-100 text-indigo-800 dark:bg-indigo-900/20 dark:text-indigo-400
                                                @else bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400 @endif">
                                                {{ ucfirst($selectedBank->tujuan) }}
                                            </span>
                                        </dd>
                                    ...
                                    <div>
                                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Status Rekening</dt>
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full
                                                @if($selectedBank->status === 'aktif') bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400
                                                @elseif($selectedBank->status === 'nonaktif') bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400
                                    ...
                                    <div>
                                        ...
                                        <dd class="mt-1">
                                            @if($selectedBank->is_primary)
                                                <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-400">
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-3 py-1 text-sm font-semibold rounded-full bg-gray-100 text-gray-800 dark:bg-gray-900/20 dark:text-gray-400">
                                                    Rekening Sekunder
                                                </span>
                                            @endif
                                        </dd>
                                    </div>
                                    @if($selectedBank->tanggal_buka)
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Tanggal Buka Rekening</dt>
                                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                                {{ \Carbon\Carbon::parse($selectedBank->tanggal_buka)->format('d F Y') }}
                                                <span class="text-xs text-gray-500 ml-2">
                                                    ({{ \Carbon\Carbon::parse($selectedBank->tanggal_buka)->locale('id')->diffForHumans() }})
                                                </span>
                                            </dd>
                                        </div>
                                    @endif
                        ...
                        <!-- Status Alert -->
                        @if($selectedBank->status === 'blocked')
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-2 text-sm text-red-700 dark:text-red-300">
                                            <p>Rekening ini saat ini dalam status diblokir. Hubungi administrator untuk informasi lebih lanjut.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-2 text-sm text-orange-700 dark:text-orange-300">
                                            <p>Rekening ini telah ditutup dan tidak dapat digunakan lagi untuk transaksi.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        @elseif($selectedBank->is_primary)
                                ...
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <div class="mt-2 text-sm text-blue-700 dark:text-blue-300">
                                            <p>Ini adalah rekening utama yang digunakan untuk pembayaran gaji dan tunjangan.</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-sm text-gray-500 dark:text-gray-400 truncate">
                                                {{ basename($selectedBank->document_path) }}
                                            </p>
                                        </div>
                                        <div class="flex-shrink-0">
                                            <a href="{{ asset('storage/' . $selectedBank->document_path) }}" 
                                               target="_blank"
                                               class="inline-flex items-center px-3 py-1 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
                        ...
                        <!-- Account Summary -->
                        <div class="mt-6 bg-gradient-to-r from-sky-50 to-blue-50 dark:from-sky-900/20 dark:to-blue-900/20 rounded-lg border border-sky-200 dark:border-sky-800 p-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                                    <div class="text-2xl font-bold text-sky-600 dark:text-sky-400">
                                        {{ strtoupper($selectedBank->kode_bank ?? 'N/A') }}
                                    </div>
                                    <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">Kode Bank</div>
                                </div>
                                <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                                    <div class="text-2xl font-bold text-sky-600 dark:text-sky-400">
                                        @if($selectedBank->jenis_rekening === 'tabungan')
                                            💳
                                        @elseif($selectedBank->jenis_rekening === 'giro')
                                            📊
                                        @elseif($selectedBank->jenis_rekening === 'deposito')
                                            💰
                                        @elseif($selectedBank->jenis_rekening === 'kredit')
                                            🏦
                                        @else
                                            💵
                                        @endif
                                    </div>
                                    <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ ucfirst($selectedBank->jenis_rekening) }}</div>
                                </div>
                                <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                                    <div class="text-2xl font-bold text-sky-600 dark:text-sky-400">
                                        @if($selectedBank->status === 'aktif')
                                    ...
                                    <div class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ ucfirst($selectedBank->status) }}</div>
                                </div>
                                <div class="text-center p-3 bg-white dark:bg-gray-800 rounded-lg">
                                    <div class="text-2xl font-bold text-sky-600 dark:text-sky-400">
                                        @if($selectedBank->is_primary)
                        ...
                        <div class="mt-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                            <div class="px-4 py-3 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                                <h4 class="font-semibold text-gray-900 dark:text-white">Informasi Transfer</h4>
                            </div>
                            <div class="p-4">
                                <div class="bg-gradient-to-r from-sky-50 to-blue-50 dark:from-sky-900/10 dark:to-blue-900/10 rounded-lg p-4 border border-sky-200 dark:border-sky-800">
                                    <div class="space-y-2">
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600 dark:text-gray-400">Bank</span>
                                            <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ $selectedBank->nama_bank }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600 dark:text-gray-400">Kode Bank</span>
                                            <span class="text-sm font-mono font-bold text-gray-900 dark:text-white">{{ $selectedBank->kode_bank }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600 dark:text-gray-400">No. Rekening</span>
                                            <span class="text-base font-mono font-bold text-sky-600 dark:text-sky-400">{{ $selectedBank->nomor_rekening }}</span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600 dark:text-gray-400">Atas Nama</span>
                                            <span class="text-sm font-semibold text-gray-900 dark:text-white">{{ $selectedBank->nama_pemilik }}</span>
                                        </div>
                                    </div>
                                    <div class="mt-3 pt-3 border-t border-sky-200 dark:border-sky-800">
                                        <p class="text-xs text-gray-500 dark:text-gray-400 text-center">
                                            💡 Gunakan informasi di atas untuk melakukan transfer ke rekening ini
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-600 dark:text-gray-100">
                                            {{ $selectedBank->created_at ? \Carbon\Carbon::parse($selectedBank->created_at)->format('d F Y, H:i') : 'N/A' }}
                                        </dd>
                                    </div>
                                    @if($selectedBank->created_by)
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Dibuat Oleh</dt>
                                            <dd class="mt-1 text-sm text-gray-600 dark:text-gray-100">
                                                {{ $selectedBank->creator?->name ?? $selectedBank->created_by }}
                                            </dd>
                                        </div>
                                    @endif
                                </div>
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <dd class="mt-1 text-sm text-gray-600 dark:text-gray-100">
                                            {{ $selectedBank->updated_at ? \Carbon\Carbon::parse($selectedBank->updated_at)->format('d F Y, H:i') : 'N/A' }}
                                        </dd>
                                    </div>
                                    @if($selectedBank->updated_by)
                                        <div>
                                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Diperbarui Oleh</dt>
                                            <dd class="mt-1 text-sm text-gray-600 dark:text-gray-100">
                                                {{ $selectedBank->updater?->name ?? $selectedBank->updated_by }}
                                            </dd>
                                        </div>
                                    @endif
                                </div>
