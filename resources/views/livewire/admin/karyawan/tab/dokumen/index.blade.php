                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-1 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('nama_dokumen')">
                            <div class="flex items-center gap-2">
                                <span>Dokumen</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'nama_dokumen')
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('jenis_dokumen')">
                            <div class="flex items-center gap-2">
                                <span>Jenis</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'jenis_dokumen')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                            wire:click="sortBy('status_dokumen')">
                            <div class="flex items-center gap-2">
                                <span>Status</span>
                                <div class="sort-icon">
                                    @if ($sortField === 'status_dokumen')
                                        @if ($sortDirection === 'asc')
                                            <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @forelse($documents as $dokumen)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 capitalize">
                                ...
                                @if ($dokumen->jenis_dokumen === 'lainnya') 
                                    {{ $dokumen->jenis_lainnya ?? '-' }}
                                @else
                                    {{ $dokumen->jenisBadge['text'] ?? '-' }}
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 capitalize">
                                <button
                                    class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $dokumen->statusBadge['class'] }}">
                                    {{ $dokumen->statusBadge['text'] }}
                                </button>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                @if ($dokumen->document_path)
                                    <!-- Tombol untuk membuka modal preview -->
                                    <button
                                        wire:click="$dispatch('preview-file', { url: '{{ Storage::url($dokumen->document_path) }}' })"
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-lg font-medium">No dokumen found</p>
                                    <p class="text-sm">Get started by creating a new dokumen.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                            ...
                            <form wire:submit.prevent="save" enctype="multipart/form-data" class="space-y-5">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <input wire:model.live="nama_dokumen" type="text"
                                            placeholder="e.g. Kartu Keluarga, etc."
                                    ...
                                    <!--Jenis Dokumen -->
                                    <div class="space-y-2">
                                        <label class="block text-sm font-medium text-gray-700 mb-1">Jenis Dokumen
                                            <span class="text-red-500">*</span></label>
                                        <div class="relative">
                                            <select wire:model.live="jenis_dokumen"
                                                ...
                                                <option value="" class="text-gray-400">Pilih Jenis Dokumen
                                                </option>
                                                @foreach (\App\Models\Employee\KaryawanDokumen::JENIS_DOKUMEN as $key => $label)
                                    ...
                                    @if ($jenis_dokumen === 'lainnya')
                                        ...
                                        <div class="space-y-2">
                                            ...
                                            <input wire:model.live="jenis_lainnya" type="text"
                                                placeholder="e.g. Surat Keterangan, etc."
                                                class="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all">
                                            @error('jenis_lainnya')
                                                <p class="text-xs text-red-500 flex items-center gap-1">
                                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
