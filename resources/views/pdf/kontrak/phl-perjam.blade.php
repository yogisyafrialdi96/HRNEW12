...
<head>
    ...
    <title>PHL PER JAM MENGAJAR - {{ $kontrak->nomor_kontrak }}</title>
    <style>
        @page {
            margin-left: 2cm;
            margin-right: 2cm;
...
<body>
    ...
    <div class="content">
        ...
        <table>
            ...
            @if ($kontrak->karyawan->alamat_ktp != $kontrak->karyawan->alamat_dom)
            <tr>
                <td></td>
            ...
            <tr>
                ...
                <td class="separator">:</td>
                <td>{{ $kontrak->karyawan->alamat_dom ?? '[Alamat Domisili]' }}</td>
            </tr>
            @else
            <tr>
                <td></td>
        ...
        <div class="pasal">
            ...
            <ol type="a">
                <li>Honor  paket  Rp.{{ number_format($kontrak->gaji_paket, 0, ',', '.') }},- (<i>{{ App\Helpers\TerbilangHelper::terbilangRupiah($kontrak->gaji_paket) }}</i>) Jam Mengajar. Honor Paket ini mencakup semua kompensasi dan maslah-maslah Pihak Kedua. Tidak ada pembayaran lain yang terhutang dari Pihak Pertama kepada Pihak Kedua.</li>
                <li>Tunjangan Operasional Rp.{{ number_format($kontrak->transport, 0, ',', '.') }},- (<i>{{ App\Helpers\TerbilangHelper::terbilangRupiah($kontrak->transport) }}</i>) per kedatangan selama kontrak.</li>
            </ol>
        </div>
